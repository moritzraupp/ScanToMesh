...
set EDITOR_SCRIPTS_DIR=UnityImplementation\Assets\EditorScripts

set OUT_DIR=Unity_out\ScanToMesh
set CS_OUT_DIR=%OUT_DIR%\CSharpHost
set PY_OUT_DIR=%OUT_DIR%\python_runtime
set SCRIPTS_OUT_DIR=%OUT_DIR%\PythonScripts
set EDITOR_SCRIPTS_OUT_DIR=%OUT_DIR%\EditorScripts
...
if not exist "%EDITOR_SCRIPTS_OUT_DIR%" mkdir "%EDITOR_SCRIPTS_OUT_DIR%"
...
REM === EditorScripts kopieren ===

if exist "%EDITOR_SCRIPTS_DIR%" (
    echo Kopiere %EDITOR_SCRIPTS_DIR% nach %EDITOR_SCRIPTS_OUT_DIR% ...
    if not exist "%EDITOR_SCRIPTS_OUT_DIR%" (
        mkdir "%EDITOR_SCRIPTS_OUT_DIR%"
    )
    xcopy /E /I /Y "%EDITOR_SCRIPTS_DIR%" "%EDITOR_SCRIPTS_OUT_DIR%"
) else (
    echo [WARNUNG] %EDITOR_SCRIPTS_DIR% nicht gefunden.
)
